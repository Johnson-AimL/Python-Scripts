...
class mainApp(wx.Frame):
    ...
    def InitUI(self):
        ...
        self.label = wx.StaticText(self, label="Welcome, RJW ELECTRICALS  - GST Grouping Program V2.0")
        ...
        self.SetTitle('RJW ELECTRICALS  - GST Grouping Program V2.0')
        self.Centre()
    ...
    def on_submit(self, event):
        ...
        file_handler = logging.FileHandler(self.directory + '/error.log')
        logger.addHandler(file_handler)

        # Set the logging level to capture all errors
        ...
        try:
            ...
            # Log the error message with the code line number
            tb = traceback.format_exc()
            logger.error(tb)

            self.label.SetLabel("An error occurred. Please try after correcting. For more details, view log file.")

            time.sleep(30)

            # Close the logger and exit the application
    ...
    def loadInput(self):
        ...
        #  self.grouped_df['Rate'] = self.grouped_df['Rate'].apply(lambda x: '{:.2f}'.format(x) if isinstance(x, (int, float)) else x)
        #  self.grouped_df['Total Quantity'] = self.grouped_df['Total Quantity'].apply(lambda x: '{:.2f}'.format(x) if isinstance(x, (int, float)) else x)
        #  self.grouped_df['Total Value'] = self.grouped_df['Total Value'].apply(lambda x: '{:.2f}'.format(x) if isinstance(x, (int, float)) else x)
        #  self.grouped_df['Taxable Value'] = self.grouped_df['Taxable Value'].apply(lambda x: '{:.2f}'.format(x) if isinstance(x, (int, float)) else x)
        #  self.grouped_df['Integrated Tax Amount'] = self.grouped_df['Integrated Tax Amount'].apply(lambda x: '{:.2f}'.format(x) if isinstance(x, (int, float)) else x)
        #  self.grouped_df['Central Tax Amount'] = self.grouped_df['Central Tax Amount'].apply(lambda x: '{:.2f}'.format(x) if isinstance(x, (int, float)) else x)
        #  self.grouped_df['State/UT Tax Amount'] = self.grouped_df['State/UT Tax Amount'].apply(lambda x: '{:.2f}'.format(x) if isinstance(x, (int, float)) else x)
        #  self.grouped_df['Cess Amount'] = self.grouped_df['Cess Amount'].apply(lambda x: '{:.2f}'.format(x) if isinstance(x, (int, float)) else x)
