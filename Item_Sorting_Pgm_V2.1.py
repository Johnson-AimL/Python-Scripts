# This is a software which compare three excel sheet, and produce mismatch report and match report.
# Designed as per Customer Needs
# Licenced for RJW Electricals Version 2.0
# Added loginc to handle both xls an xlsx format for vyapar.
...
class mainApp(wx.Frame):
    ...
    def InitUI(self):
        ...
        self.label = wx.StaticText(self, label="Welcome, RJW ELECTRICALS  - Stock Matching Program V2.1")
        ...
        self.SetTitle('RJW ELECTRICALS  - Stock Matching Program V2.1')
        self.Centre()
    ...
    def loadVyapar(self):
        ...
        if self.VyaparFile_path.endswith(".xlsx"):
            # Load the Excel file into a variable
            workbookT = openpyxl.load_workbook(self.VyaparFile_path)
            ...
            for i, row in enumerate(sheetV.iter_rows(values_only=True)):
                ...
                if (row[0] and row[1] and row[0] != "Item Name"):
                    ...
                    self.vyaparitem_df = self.vyaparitem_df.append({"Item": row[0], "Quantity": row[1]},
                                                                   ignore_index=True)


        #         workbookT = pd.read_excel(self.VyaparFile_path)
        #         sheetV = workbookT.active
        elif self.VyaparFile_path.endswith(".xls"):
            workbookT = xlrd.open_workbook(self.VyaparFile_path)
            #        workbookT = pd.read_excel(self.VyaparFile_path, engine='xlrd')
            ...
            raise ValueError("File format not supported. Please provide an .xls or .xlsx file for Vyapar.")
    ...
    def getcommonlist(self):
        ...
        #############################################################################################################################
        ################  All 3 values were different   #############################################################################
        #############################################################################################################################

        self.not_matching_items1 = self.result[(self.result['Tally_Value'] != self.result['Stock_Excel_Value']) & (
                    self.result['Tally_Value'] != self.result['Vyapar_Value']) & (
                                                           self.result['Stock_Excel_Value'] != self.result[
                                                       'Vyapar_Value'])]
        self.not_matching_items1 = self.not_matching_items1[
            ['Item', 'Tally_Value', 'Stock_Excel_Value', 'Vyapar_Value']]

        #############################################################################################################################
        ################  Tally and stock matching and vyapar values were different   ###############################################
        #############################################################################################################################

        self.not_matching_items2 = self.result[(self.result['Tally_Value'] == self.result['Stock_Excel_Value']) & (
                    self.result['Tally_Value'] != self.result['Vyapar_Value'])]
        self.not_matching_items2 = self.not_matching_items2[
            ['Item', 'Tally_Value', 'Stock_Excel_Value', 'Vyapar_Value']]

        #############################################################################################################################
        ################  vyapar and stock matching and tally values were different   ###############################################
        #############################################################################################################################

        self.not_matching_items3 = self.result[(self.result['Vyapar_Value'] == self.result['Stock_Excel_Value']) & (
                    self.result['Tally_Value'] != self.result['Vyapar_Value'])]
        self.not_matching_items3 = self.not_matching_items3[
            ['Item', 'Tally_Value', 'Stock_Excel_Value', 'Vyapar_Value']]

        #############################################################################################################################
        ################  Tally and vyapar matching and stock values were different   ###############################################
        #############################################################################################################################

        self.not_matching_items4 = self.result[(self.result['Vyapar_Value'] == self.result['Tally_Value']) & (
                    self.result['Stock_Excel_Value'] != self.result['Vyapar_Value'])]
        self.not_matching_items4 = self.not_matching_items4[
            ['Item', 'Tally_Value', 'Stock_Excel_Value', 'Vyapar_Value']]
    ...
    def writeoutput(self):
        ...
        # Hide the worksheet if the data is empty
        if self.merged_df.empty:
            merged_dfSh.sheet_state = 'hidden'
            ...
            for cell in row:
                if isinstance(cell.value, (int, float)) and cell.value < 0:
                    ...
                    cell.border = Border(left=Side(style='thin'),
                                         ...
                                         bottom=Side(style='thin'))
        # Hide the worksheet if the data is empty
        if self.tallyitem_dfD.empty:
            tlydupitemSh.sheet_state = 'hidden'
        ...
        # Hide the worksheet if the data is empty
        if self.Stockitem_dfD.empty:
            stkdupitemSh.sheet_state = 'hidden'
        ...
        # Hide the worksheet if the data is empty
        if self.vyaparitem_dfD.empty:
            vprdupitemSh.sheet_state = 'hidden'

        ################################################################################################################################
        #  Writing unique items with values matching, formatting it in the excel.
        #
        ###############################################################################################################################
        self.update_progress(94)
        ...
        # Hide the worksheet if the data is empty
        if self.matching_items.empty:
            unqmtchitemSh.sheet_state = 'hidden'
        ...
        self.not_matching_items1.to_excel(writer, index=False, sheet_name='All_3_Mismatch')

        # Get a reference to the worksheet
        unqmismtchitemSh = writer.book['All_3_Mismatch']
        ...
        for col_num, value in enumerate(self.not_matching_items1.columns.values):
        ...
        # Hide the worksheet if the data is empty
        if self.not_matching_items1.empty:
            unqmismtchitemSh.sheet_state = 'hidden'

        ###########################################

        self.update_progress(98)
        self.not_matching_items2.to_excel(writer, index=False, sheet_name='Vyapar_Mismatch')

        # Get a reference to the worksheet
        unqmismtchitemSh2 = writer.book['Vyapar_Mismatch']

        # Set the column widths
        unqmismtchitemSh2.column_dimensions['A'].width = 50
        unqmismtchitemSh2.column_dimensions['B'].width = 20
        unqmismtchitemSh2.column_dimensions['C'].width = 20
        unqmismtchitemSh2.column_dimensions['D'].width = 20

        for col_num, value in enumerate(self.not_matching_items2.columns.values):
            cell = unqmismtchitemSh2.cell(row=1, column=col_num + 1)
            cell.value = value
            cell.font = header_font
            cell.fill = header_fill
            cell.alignment = Alignment(horizontal='center')
        ...
        # Iterate through each cell in the worksheet
        for row in unqmismtchitemSh2.iter_rows():
            for cell in row:
                if isinstance(cell.value, (int, float)) and cell.value < 0:
                    cell.alignment = Alignment(horizontal='center', vertical='center')
                    cell.fill = PatternFill(start_color='ADD8E6', end_color='ADD8E6', fill_type='solid')
        ...
        # Hide the worksheet if the data is empty
        if self.not_matching_items2.empty:
            unqmismtchitemSh2.sheet_state = 'hidden'
        ...
        self.not_matching_items3.to_excel(writer, index=False, sheet_name='Tally_Mismatch')

        # Get a reference to the worksheet
        unqmismtchitemSh3 = writer.book['Tally_Mismatch']

        # Set the column widths
        unqmismtchitemSh3.column_dimensions['A'].width = 50
        unqmismtchitemSh3.column_dimensions['B'].width = 20
        unqmismtchitemSh3.column_dimensions['C'].width = 20
        unqmismtchitemSh3.column_dimensions['D'].width = 20

        for col_num, value in enumerate(self.not_matching_items3.columns.values):
            cell = unqmismtchitemSh3.cell(row=1, column=col_num + 1)
            cell.value = value
            cell.font = header_font
            cell.fill = header_fill
            cell.alignment = Alignment(horizontal='center')
        ...
        # Iterate through each cell in the worksheet
        for row in unqmismtchitemSh3.iter_rows():
            for cell in row:
                if isinstance(cell.value, (int, float)) and cell.value < 0:
                    cell.alignment = Alignment(horizontal='center', vertical='center')
                    cell.fill = PatternFill(start_color='ADD8E6', end_color='ADD8E6', fill_type='solid')
        ...
        # Hide the worksheet if the data is empty
        if self.not_matching_items3.empty:
            unqmismtchitemSh3.sheet_state = 'hidden'

        ###########################################

        ###########################################

        self.not_matching_items4.to_excel(writer, index=False, sheet_name='StockXL_Mismatch')

        # Get a reference to the worksheet
        unqmismtchitemSh4 = writer.book['StockXL_Mismatch']

        # Set the column widths
        unqmismtchitemSh4.column_dimensions['A'].width = 50
        unqmismtchitemSh4.column_dimensions['B'].width = 20
        unqmismtchitemSh4.column_dimensions['C'].width = 20
        unqmismtchitemSh4.column_dimensions['D'].width = 20

        for col_num, value in enumerate(self.not_matching_items4.columns.values):
            cell = unqmismtchitemSh4.cell(row=1, column=col_num + 1)
            cell.value = value
            cell.font = header_font
            cell.fill = header_fill
            cell.alignment = Alignment(horizontal='center')
        ...
        # Iterate through each cell in the worksheet
        for row in unqmismtchitemSh4.iter_rows():
            for cell in row:
                if isinstance(cell.value, (int, float)) and cell.value < 0:
                    cell.alignment = Alignment(horizontal='center', vertical='center')
                    cell.fill = PatternFill(start_color='ADD8E6', end_color='ADD8E6', fill_type='solid')
        ...
        # Hide the worksheet if the data is empty
        if self.not_matching_items4.empty:
            unqmismtchitemSh4.sheet_state = 'hidden'
    ...
    def on_submit(self, event):
        ...
        self.StockFile_path = self.file1.GetPath()
        self.TallyFile_path = self.file2.GetPath()
        self.VyaparFile_path = self.file3.GetPath()

        self.directory = os.path.dirname(self.StockFile_path)
